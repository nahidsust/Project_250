...
public class MainActivity extends AppCompatActivity {
    ...
    private void setupSearchView() {
        ...
        // Get the EditText inside SearchView safely
        ...
        new androidx.appcompat.app.AlertDialog.Builder(this)
                ...
                .setPositiveButton("Delete", (dialog, which) -> {
                    ...
                    deleteUserData(userId, () -> {
                        // Then delete the Firebase Authentication user account
                        user.delete()
                    ...
                    if (task.isSuccessful()) {
                        ...
                        database.child("user").child(userId).removeValue()
                                .addOnCompleteListener(task2 -> {
                                    onComplete.run(); // proceed to delete user account
                                });
                    } else {
                        // If failed to get posts, still try to delete user profile data
                        database.child("user").child(userId).removeValue()
                                .addOnCompleteListener(task2 -> {
                                    onComplete.run();
                                });
